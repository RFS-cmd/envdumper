...
-- TEMA AZUL NEON - FACILMENTE MODIFICÃƒÆ’Ã‚ÂVEL
local ThemeColors = {
    ...
    SecondaryBg = Color3.fromRGB(15, 20, 30),     -- Fundo secundÃƒÆ’Ã‚Â¡rio
    TertiaryBg = Color3.fromRGB(25, 30, 40),      -- Fundo terciÃƒÆ’Ã‚Â¡rio
    Border = Color3.fromRGB(40, 50, 70),          -- Bordas
    
    -- Cores de texto
    TextPrimary = Color3.fromRGB(240, 245, 255),  -- Texto primÃƒÆ’Ã‚Â¡rio
    TextSecondary = Color3.fromRGB(180, 200, 230),-- Texto secundÃƒÆ’Ã‚Â¡rio
...
local ThemePresets = {
    ["Neon Blue"] = {
        ...
        TertiaryBg = Color3.fromRGB(25, 30, 40),
    ...
    ["Crimson"] = {
        Primary = Color3.fromRGB(255, 75, 95),
        PrimaryDark = Color3.fromRGB(170, 40, 60),
        PrimaryLight = Color3.fromRGB(255, 150, 165),
        Accent = Color3.fromRGB(255, 120, 140),
        Background = Color3.fromRGB(18, 10, 12),
        SecondaryBg = Color3.fromRGB(28, 14, 19),
        TertiaryBg = Color3.fromRGB(36, 20, 26),
        Border = Color3.fromRGB(75, 38, 49),
        TextPrimary = Color3.fromRGB(255, 240, 244),
        TextSecondary = Color3.fromRGB(232, 182, 193),
        TextDisabled = Color3.fromRGB(149, 107, 118),
        Success = Color3.fromRGB(75, 220, 140),
        Error = Color3.fromRGB(255, 85, 85),
        Warning = Color3.fromRGB(255, 190, 75)
    },
    ["Emerald"] = {
        Primary = Color3.fromRGB(40, 205, 140),
        PrimaryDark = Color3.fromRGB(25, 125, 86),
        PrimaryLight = Color3.fromRGB(120, 240, 190),
        Accent = Color3.fromRGB(80, 225, 170),
        Background = Color3.fromRGB(9, 15, 12),
        SecondaryBg = Color3.fromRGB(14, 24, 19),
        TertiaryBg = Color3.fromRGB(22, 34, 28),
        Border = Color3.fromRGB(42, 72, 60),
        TextPrimary = Color3.fromRGB(236, 255, 246),
        TextSecondary = Color3.fromRGB(175, 221, 200),
        TextDisabled = Color3.fromRGB(102, 142, 124),
        Success = Color3.fromRGB(70, 230, 120),
        Error = Color3.fromRGB(255, 95, 95),
        Warning = Color3.fromRGB(255, 210, 90)
    },
    ["Sunset"] = {
        Primary = Color3.fromRGB(255, 155, 70),
        PrimaryDark = Color3.fromRGB(180, 95, 35),
        PrimaryLight = Color3.fromRGB(255, 205, 140),
        Accent = Color3.fromRGB(255, 180, 100),
        Background = Color3.fromRGB(19, 13, 9),
        SecondaryBg = Color3.fromRGB(29, 19, 13),
        TertiaryBg = Color3.fromRGB(40, 26, 18),
        Border = Color3.fromRGB(77, 53, 37),
        TextPrimary = Color3.fromRGB(255, 246, 234),
        TextSecondary = Color3.fromRGB(233, 201, 170),
        TextDisabled = Color3.fromRGB(155, 130, 104),
...
-- Sistema de notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes fixado
local function CreateNotificationContainer()
    local NotificationContainer = Instance.new("Frame")
    NotificationContainer.Name = "NotificationContainer"
    NotificationContainer.Size = UDim2.new(0, 300, 0, 0)
    NotificationContainer.Position = UDim2.new(1, -310, 0, 10) -- PosiÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o fixa no canto superior direito
...
-- Garantir que o container de notificaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes exista
...
function Config.load(file_name, config)
    local success_load, result = pcall(function()
        if not isfolder("River") then
            makefolder("River")
        end

        local path = "River/" .. file_name .. ".json"
...
local function ensure_config_shape(config)
    config = config or {}
...
local function get_profile_folder(game_id)
    local root = "River/" .. game_id
...
local function get_profile_path(game_id, profile_name)
    return get_profile_folder(game_id) .. "/" .. sanitize_profile_name(profile_name) .. ".json"
end

local Library = {
    _config = ensure_config_shape(Config.load(tostring(game.GameId), {
        _flags = {},
        _keybinds = {},
        _library = {}
    })),
...
do
    local lib_cfg = Library._config._library
    if lib_cfg.autoload and lib_cfg.autoload_profile then
        local auto_name = sanitize_profile_name(lib_cfg.autoload_profile)
        local auto_path = get_profile_path(tostring(game.GameId), auto_name)

        if isfile(auto_path) then
            local ok, loaded = pcall(function()
                return HttpService:JSONDecode(readfile(auto_path))
            end)

            if ok and typeof(loaded) == "table" then
                Library._config = ensure_config_shape(loaded)
                Library._config._library.autoload = true
                Library._config._library.autoload_profile = auto_name
                Library._config._library.selected_profile = auto_name
...
local function normalize_icon_input(icon_name_or_id)
    if typeof(icon_name_or_id) ~= "string" then
        return icon_name_or_id
    end

    local trimmed = string.match(icon_name_or_id, "^%s*(.-)%s*$")
    if trimmed == "" then
        return nil
    end
...
function Library.new(settings)
    ...
    local self = setmetatable({
        _loaded = false,
        _tab = 0,
        _window_title = settings.title or settings.name or "River",
        _window_icon = normalize_icon_input(settings.icon),
    }, Library)

    local preferred_theme = Library._config._library.theme
    if typeof(preferred_theme) ~= "string" or not ThemePresets[preferred_theme] then
        preferred_theme = "Crimson"
    end
    self:apply_theme(preferred_theme, false, false)

    self:create_ui()

    if settings.title or settings.name or settings.icon then
        self:set_branding(self._window_title, self._window_icon)
    end
...
function Library:apply_theme(theme_name, repaint_existing_ui, persist_config)
    local selected_theme = (typeof(theme_name) == "string" and ThemePresets[theme_name]) and theme_name or "Crimson"
    local preset = ThemePresets[selected_theme]
    ...
    Library._config._library.theme = selected_theme
    self._config = Library._config

    local function map_color(color)
        if typeof(color) ~= "Color3" then
            return color
        end

        for key, old_value in pairs(old_theme) do
            if old_value == color and ThemeColors[key] then
    ...
    if repaint_existing_ui and self._ui then
        local all_objects = self._ui:GetDescendants()
        table.insert(all_objects, self._ui)

        for _, object in ipairs(all_objects) do
            pcall(function()
                object.BackgroundColor3 = map_color(object.BackgroundColor3)
            end)
            pcall(function()
                object.TextColor3 = map_color(object.TextColor3)
            end)
            pcall(function()
                object.ImageColor3 = map_color(object.ImageColor3)
            end)
            pcall(function()
                object.BorderColor3 = map_color(object.BorderColor3)
            end)
            pcall(function()
                object.ScrollBarImageColor3 = map_color(object.ScrollBarImageColor3)
            end)
            pcall(function()
                object.PlaceholderColor3 = map_color(object.PlaceholderColor3)
            end)
            pcall(function()
                object.Color = map_color(object.Color)
            end)
            pcall(function()
                local sequence = object.Color
                if typeof(sequence) == "ColorSequence" then
                    local keypoints = {}
                    for _, keypoint in ipairs(sequence.Keypoints) do
                        table.insert(keypoints, ColorSequenceKeypoint.new(keypoint.Time, map_color(keypoint.Value)))
                    end
...
function Library:get_profile_list()
    local profiles = {}
    local folder = get_profile_folder(tostring(game.GameId))
    ...
    for _, file_path in ipairs(files) do
        local profile_name = tostring(file_path):match("([^\\/]+)%.json$")
...
function Library:save_profile(profile_name)
    local game_id = tostring(game.GameId)
    local normalized_name = sanitize_profile_name(profile_name)
    local target_path = get_profile_path(game_id, normalized_name)

    local success, result = pcall(function()
        writefile(target_path, HttpService:JSONEncode(Library._config))
    end)

    if not success then
        return false, normalized_name, result
    end

    Library._config._library.selected_profile = normalized_name
    if Library._config._library.autoload then
        Library._config._library.autoload_profile = normalized_name
    end
    Config.save(game_id, Library._config)

    return true, normalized_name
end

function Library:load_profile(profile_name)
    local game_id = tostring(game.GameId)
    local normalized_name = sanitize_profile_name(profile_name)
    local target_path = get_profile_path(game_id, normalized_name)

    if not isfile(target_path) then
        return false, normalized_name, "profile_not_found"
    end

    local success, loaded = pcall(function()
        return HttpService:JSONDecode(readfile(target_path))
    end)

    if not success or typeof(loaded) ~= "table" then
        return false, normalized_name, loaded
    end

    local previous_library = Library._config._library or {}

    loaded = ensure_config_shape(loaded)
    loaded._library.selected_profile = normalized_name

    if previous_library.autoload then
        loaded._library.autoload = true
        loaded._library.autoload_profile = previous_library.autoload_profile or normalized_name
    end
...
function Library.SendNotification(settings)
    ...
    -- Ajustar tamanho do InnerFrame apÃƒÆ’Ã‚Â³s o texto ser renderizado
    task.spawn(function()
        wait(0.05)
...
-- FUNÃƒÆ’Ã¢â‚¬Â¡ÃƒÆ’Ã†â€™O PARA CRIAR DROPDOWN COM REFRESH AUTOMÃƒÆ’Ã‚ÂTICO
function Library:create_refresh_dropdown(settings)
    ...
    -- FunÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o para atualizar as opÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes do dropdown
    ...
    -- FunÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o para atualizar a exibiÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o do dropdown
    function DropdownManager:update_display()
        ...
        -- Adicionar novas opÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Âµes
    ...
    -- Iniciar refresh automÃƒÆ’Ã‚Â¡tico se especificado
    ...
    -- MÃƒÆ’Ã‚Â©todo para parar o refresh
    function DropdownManager:stop_refresh()
        self._refresh_running = false
    end

    -- MÃƒÆ’Ã‚Â©todo para iniciar o refresh
...
function Library:create_ui()
    ...
    local function resolve_icon(icon_name_or_id, fallback)
        icon_name_or_id = normalize_icon_input(icon_name_or_id)
        if typeof(icon_name_or_id) ~= "string" then
            return icon_name_or_id or fallback
        end

        local lowered = string.lower(icon_name_or_id)
        if LucideIcons[lowered] then
            return LucideIcons[lowered]
        end

        if string.sub(lowered, 1, 11) == "rbxassetid://" then
            return icon_name_or_id
        end

        if tonumber(icon_name_or_id) then
            return "rbxassetid://" .. icon_name_or_id
        end

        return fallback or icon_name_or_id
    end
    ...
    ClientName.Text = self._window_title or 'River'
    ...
    Icon.Image = resolve_icon(self._window_icon, nil) or ""
    ...
    local function refresh_branding_layout()
        local has_icon = Icon.Image ~= nil and Icon.Image ~= ""
        Icon.Visible = has_icon
        ClientName.Position = has_icon and UDim2.new(0.056, 0, 0.055, 0) or UDim2.new(0.025, 0, 0.055, 0)
    end

    refresh_branding_layout()
    ...
    function self:set_branding(title, icon_name_or_id)
        if typeof(title) == "string" and title ~= "" then
            self._window_title = title
            ClientName.Text = title
        end

        self._window_icon = normalize_icon_input(icon_name_or_id)
        Icon.Image = resolve_icon(self._window_icon, nil) or ""
        refresh_branding_layout()
    end
    ...
    DragArea.Size = UDim2.new(1, -90, 0, 52)
    DragArea.Position = UDim2.new(0, 50, 0, 0)
    DragArea.Parent = Handler
    ...
    Connections['container_input_began'] = DragArea.InputBegan:Connect(on_drag)
    ...
    function self:update_tabs(tab)
        for _, object in Tabs:GetChildren() do
            ...
            if object == tab then
                local pin_target_position
                if self._ui_loaded and object.AbsoluteSize.Y > 0 and Handler.AbsoluteSize.Y > 0 then
                    local relative_y = object.AbsolutePosition.Y - Handler.AbsolutePosition.Y + ((object.AbsoluteSize.Y - Pin.AbsoluteSize.Y) * 0.5)
                    pin_target_position = UDim2.new(Pin.Position.X.Scale, Pin.Position.X.Offset, 0, relative_y)
                else
                    local offset = object.LayoutOrder * (0.113 / 1.3)
                    pin_target_position = UDim2.fromScale(0.026, 0.135 + offset)
                end

                TweenService:Create(Pin, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    Position = pin_target_position
                }):Play()
    ...
    function self:create_tab(title, icon, tab_settings)
        local TabManager = {}
        tab_settings = tab_settings or {}
        local is_bottom_tab = tab_settings.bottom == true

        local function resolve_tab_icon(icon_name_or_id)
            if typeof(icon_name_or_id) ~= "string" then
                return icon_name_or_id
            end
        ...
        local has_non_bottom_tabs = false
        for _, object in Tabs:GetChildren() do
            if object.Name ~= "Tab" then
                continue
            end
            ...
            has_tabs = true
            if object ~= self._bottom_tab_button then
                has_non_bottom_tabs = true
            end
        end

        local first_tab = (not has_tabs) or ((not is_bottom_tab) and (not has_non_bottom_tabs))
        if tab_settings.no_auto_select then
            first_tab = false
        end
        ...
        Icon2.Name = 'Icon'
        Icon2.Image = resolve_tab_icon(icon or "layout-grid")
        ...
        LeftSection.ScrollBarImageColor3 = ThemeColors.Primary
        LeftSection.ScrollingDirection = Enum.ScrollingDirection.Y
        LeftSection.ScrollingEnabled = true
        ...
        RightSection.ScrollBarImageColor3 = ThemeColors.Primary
        RightSection.ScrollingDirection = Enum.ScrollingDirection.Y
        RightSection.ScrollingEnabled = true
        ...
        local function bind_section_canvas(section, layout, key_prefix)
            local function update_canvas()
                section.CanvasSize = UDim2.fromOffset(0, layout.AbsoluteContentSize.Y + 8)
            end

            update_canvas()
            Connections['section_canvas_' .. key_prefix] = layout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(update_canvas)
        end

        bind_section_canvas(LeftSection, UIListLayout2, tostring(self._tab) .. '_left')
        bind_section_canvas(RightSection, UIListLayout3, tostring(self._tab) .. '_right')

        self._tab = self._tab + 1

        if is_bottom_tab then
            self._bottom_tab_button = Tab
            Tab.LayoutOrder = self._tab
            self._tab = self._tab - 1
        elseif self._bottom_tab_button and self._bottom_tab_button ~= Tab then
            self._bottom_tab_button.LayoutOrder = self._tab
        end
        ...
        function TabManager:set_order(order)
            if typeof(order) ~= "number" then
                return
            end
            Tab.LayoutOrder = math.floor(order)
        end
        ...
        function TabManager:create_module(settings)
            ...
            local ModuleManager = {
                ...
                _size = 0,
                _multiplier = 0,
                _control_sync_handlers = {}
            }

            local description_text = settings.description
            if description_text == nil then
                description_text = "Module Description"
            end

            local has_description = typeof(description_text) ~= "string" or description_text:match("%S") ~= nil
            local HeaderHeight = has_description and 85 or 70
            local TitleY = has_description and 0.24 or 0.32
            local IconY = has_description and 0.82 or 0.72
            local ToggleY = has_description and 0.757 or 0.70
            local KeybindY = has_description and 0.735 or 0.68
            local DividerTopY = has_description and 0.62 or 0.50
            ...
            Module.Name = 'Module'
            Module.Size = UDim2.new(0, 241, 0, HeaderHeight)
            ...
            Header.Name = 'Header'
            Header.Size = UDim2.new(0, 241, 0, HeaderHeight)
            ...
            Icon3.Position = UDim2.new(0.071, 0, IconY, 0)
            Icon3.Name = 'Icon'
            ...
            ModuleName.Position = UDim2.new(0.073, 0, TitleY, 0)
            ...
            Description.Visible = has_description
            Description.Parent = Header
            
            local Toggle = Instance.new('Frame')
            Toggle.Name = 'Toggle'
            Toggle.BackgroundTransparency = 0.8
            Toggle.Position = UDim2.new(0.82, 0, ToggleY, 0)
            ...
            ToggleStroke.Color = ThemeColors.Primary
            ...
            local ToggleKnobPadding = 2
            local ToggleKnobOffPosition = UDim2.new(0, ToggleKnobPadding, 0.5, 0)
            local ToggleKnobOnPosition = UDim2.new(0, Toggle.Size.X.Offset - Circle.Size.X.Offset - ToggleKnobPadding, 0.5, 0)
            Circle.Position = ToggleKnobOffPosition
            ...
            Keybind.Position = UDim2.new(0.15, 0, KeybindY, 0)
            ...
            Divider2.Position = UDim2.new(0.5, 0, DividerTopY, 0)
            ...
            function ModuleManager:register_control_sync(handler)
                if typeof(handler) ~= "function" then
                    return
                end
                table.insert(self._control_sync_handlers, handler)
            end

            local function sync_controls_with_module_state()
                for _, handler in ipairs(ModuleManager._control_sync_handlers) do
                    pcall(handler, ModuleManager._state)
                end
            end
            ...
            function ModuleManager:change_state(state)
                ...
                if self._state then
                    TweenService:Create(Module, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                        Size = UDim2.fromOffset(241, HeaderHeight + self._size + self._multiplier)
                    }):Play()
                    ...
                    TweenService:Create(Circle, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                        ...
                        Position = ToggleKnobOnPosition
                    }):Play()
                else
                    TweenService:Create(Module, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                        Size = UDim2.fromOffset(241, HeaderHeight)
                    }):Play()
                    ...
                    TweenService:Create(Circle, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                        ...
                        Position = ToggleKnobOffPosition
                    }):Play()
                end
                ...
                do
                    local callback = settings and settings.callback
                    if type(callback) == "function" then
                        local ok, err = pcall(callback, self._state)
                        if not ok then
                            warn("module callback error:", err)
                        end
                    end
                end

                sync_controls_with_module_state()
            end
            ...
            if Library:flag_type(settings.flag, 'boolean') then
                ...
                do
                    local callback = settings and settings.callback
                    if type(callback) == "function" then
                        local ok, err = pcall(callback, ModuleManager._state)
                        if not ok then
                            warn("module init callback error:", err)
                        end
                    end
                end
                ...
                if ModuleManager._state then
                    ...
                    Circle.Position = ToggleKnobOnPosition
            ...
            -- MODULO DE PALETA DE CORES
            function ModuleManager:create_color_picker(settings)
                LayoutOrderModule = LayoutOrderModule + 1

                local stored_color = Library._config._flags[settings.flag]
                if typeof(stored_color) ~= "Color3" then
                    stored_color = settings.default or Color3.fromRGB(255, 255, 255)
                end

                local hue, sat, val = stored_color:ToHSV()
                ...
                if ModuleManager._state then
                    Module.Size = UDim2.fromOffset(241, HeaderHeight + self._size)
                end
                ...
                ColorPickerFrame.Size = UDim2.fromOffset(207, 196)
                ...
                TitleLabel.Size = UDim2.fromOffset(207, 13)
                ...
                ColorMap.Size = UDim2.fromOffset(142, 96)
                ColorMap.Position = UDim2.fromOffset(0, 20)
                ColorMap.BackgroundColor3 = Color3.fromHSV(ColorPickerManager._hue, 1, 1)
                ...
                local ColorMapStroke = Instance.new("UIStroke")
                ColorMapStroke.Color = ThemeColors.Primary
                ColorMapStroke.Transparency = 0.4
                ColorMapStroke.Thickness = 1
                ColorMapStroke.Parent = ColorMap
                ...
                ValueGradient.Color = ColorSequence.new(Color3.new(0, 0, 0), Color3.new(0, 0, 0))
                ...
                ColorSelector.Size = UDim2.fromOffset(8, 8)
                ...
                HueBar.Name = "HueBar"
                HueBar.Size = UDim2.fromOffset(16, 96)
                HueBar.Position = UDim2.fromOffset(147, 20)
                ...
                HueStroke.Color = ThemeColors.Primary
                HueStroke.Transparency = 0.4
                ...
                Preview.Position = UDim2.fromOffset(0, 126)
                Preview.BackgroundColor3 = ColorPickerManager._color
                ...
                PreviewStroke.Color = ThemeColors.Primary
                PreviewStroke.Transparency = 0.35
                PreviewStroke.Thickness = 1
                PreviewStroke.Parent = Preview

                local RGBLabel = Instance.new("TextLabel")
                RGBLabel.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium, Enum.FontStyle.Normal)
                RGBLabel.TextSize = 10
                RGBLabel.TextColor3 = ThemeColors.TextSecondary
                RGBLabel.TextXAlignment = Enum.TextXAlignment.Left
                RGBLabel.BackgroundTransparency = 1
                RGBLabel.Size = UDim2.fromOffset(80, 22)
                RGBLabel.Position = UDim2.fromOffset(28, 126)
                RGBLabel.Parent = ColorPickerFrame

                local HexInput = Instance.new("TextBox")
                HexInput.Name = "HexInput"
                HexInput.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium, Enum.FontStyle.Normal)
                HexInput.TextSize = 10
                HexInput.TextColor3 = ThemeColors.TextPrimary
                HexInput.PlaceholderColor3 = ThemeColors.TextDisabled
                HexInput.PlaceholderText = "#00AFFF"
                HexInput.ClearTextOnFocus = false
                HexInput.TextXAlignment = Enum.TextXAlignment.Left
                HexInput.BackgroundColor3 = ThemeColors.SecondaryBg
                HexInput.BackgroundTransparency = 0.15
                HexInput.BorderSizePixel = 0
                HexInput.Size = UDim2.fromOffset(95, 22)
                HexInput.Position = UDim2.fromOffset(112, 126)
                ...
                HexStroke.Color = ThemeColors.Primary
                HexStroke.Transparency = 0.35
                ...
                Hint.TextSize = 9
                Hint.TextColor3 = ThemeColors.TextDisabled
                Hint.Text = "Drag picker, drag hue, or type hex"
                Hint.TextXAlignment = Enum.TextXAlignment.Left
                Hint.BackgroundTransparency = 1
                Hint.Size = UDim2.fromOffset(207, 14)
                Hint.Position = UDim2.fromOffset(0, 154)
                ...
                local function hex_to_color(hex)
                    ...
                    if #clean ~= 6 or not clean:match("^[0-9A-F]+$") then
                        return nil
                    end
                ...
                function ColorPickerManager:commit(should_callback)
                    self._hue = math.clamp(self._hue, 0, 1)
                    self._saturation = math.clamp(self._saturation, 0, 1)
                    self._value = math.clamp(self._value, 0, 1)

                    self._color = Color3.fromHSV(self._hue, self._saturation, self._value)

                    ColorMap.BackgroundColor3 = Color3.fromHSV(self._hue, 1, 1)
                    ColorSelector.Position = UDim2.fromScale(self._saturation, 1 - self._value)
                    HueSelector.Position = UDim2.new(0, 0, self._hue, 0)
                    Preview.BackgroundColor3 = self._color

                    RGBLabel.Text = string.format(
                        "%d, %d, %d",
                        math.floor(self._color.R * 255 + 0.5),
                        math.floor(self._color.G * 255 + 0.5),
                        math.floor(self._color.B * 255 + 0.5)
                    )

                    if not HexInput:IsFocused() then
                        HexInput.Text = color_to_hex(self._color)
                    end

                    Library._config._flags[settings.flag] = self._color
                    Config.save(tostring(game.GameId), Library._config)

                    if should_callback then
                        local callback = settings and settings.callback
                        if type(callback) == "function" then
                            local ok, err = pcall(callback, self._color)
                            if not ok then
                                warn("color picker callback error:", err)
                            end
                        end
                    end
                end

                function ColorPickerManager:update_color_from_position(x, y, should_callback)
                    local rel_x = math.clamp((x - ColorMap.AbsolutePosition.X) / math.max(ColorMap.AbsoluteSize.X, 1), 0, 1)
                    local rel_y = math.clamp((y - ColorMap.AbsolutePosition.Y) / math.max(ColorMap.AbsoluteSize.Y, 1), 0, 1)
                    self._saturation = rel_x
                    self._value = 1 - rel_y
                    self:commit(should_callback)
                end

                function ColorPickerManager:update_hue_from_position(y, should_callback)
                    local rel_y = math.clamp((y - HueBar.AbsolutePosition.Y) / math.max(HueBar.AbsoluteSize.Y, 1), 0, 1)
                    self._hue = rel_y
                    self:commit(should_callback)
                end

                function ColorPickerManager:set_color(color, should_callback)
                    if typeof(color) ~= "Color3" then
                        return
                    end

                    local new_h, new_s, new_v = color:ToHSV()
                    self._hue = new_h
                    self._saturation = new_s
                    self._value = new_v
                    self:commit(should_callback ~= false)
                end
                ...
                local function begin_color_drag(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseButton1 and input.UserInputType ~= Enum.UserInputType.Touch then
                        return
                    end
                    ColorPickerManager._dragging_color = true
                    ColorPickerManager:update_color_from_position(input.Position.X, input.Position.Y, true)
                end

                local function begin_hue_drag(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseButton1 and input.UserInputType ~= Enum.UserInputType.Touch then
                        return
                    end
                    ColorPickerManager._dragging_hue = true
                    ColorPickerManager:update_hue_from_position(input.Position.Y, true)
                end

                ColorMap.InputBegan:Connect(begin_color_drag)
                HueBar.InputBegan:Connect(begin_hue_drag)

                local changed_connection_name = "color_picker_changed_" .. settings.flag
                local ended_connection_name = "color_picker_ended_" .. settings.flag
                Connections.disconnect(changed_connection_name)
                Connections.disconnect(ended_connection_name)

                Connections[changed_connection_name] = UserInputService.InputChanged:Connect(function(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseMovement and input.UserInputType ~= Enum.UserInputType.Touch then
                        return
                    end

                    if ColorPickerManager._dragging_color then
                        ColorPickerManager:update_color_from_position(input.Position.X, input.Position.Y, true)
                    end

                    if ColorPickerManager._dragging_hue then
                        ColorPickerManager:update_hue_from_position(input.Position.Y, true)
                    end
                end)

                Connections[ended_connection_name] = UserInputService.InputEnded:Connect(function(input)
                    if input.UserInputType ~= Enum.UserInputType.MouseButton1 and input.UserInputType ~= Enum.UserInputType.Touch then
                        return
                    end

                    ColorPickerManager._dragging_color = false
                    ColorPickerManager._dragging_hue = false
                end)

                HexInput.FocusLost:Connect(function()
                    local parsed = hex_to_color(HexInput.Text)
                    if parsed then
                        ColorPickerManager:set_color(parsed, true)
                        return
                    end

                    HexInput.Text = color_to_hex(ColorPickerManager._color)
                end)

                ColorPickerManager:commit(false)
            ...
            function ModuleManager:create_textbox(settings)
                ...
                local stored_text = Library._config._flags[settings.flag]
                if typeof(stored_text) ~= "string" then
                    stored_text = settings.default or ""
                end
                ...
                TextboxFrame.Size = UDim2.fromOffset(207, 40)
                TextboxFrame.BackgroundTransparency = 1
                TextboxFrame.Parent = Options
                TextboxFrame.LayoutOrder = LayoutOrderModule
                ...
                local placeholder_text = settings.placeholder or "Enter text..."
                ...
                Textbox.Text = TextboxManager._text
                Textbox.Size = UDim2.fromOffset(207, 22)
                ...
                UIStrokeTextBox.Color = Color3.fromRGB(85, 85, 85)
                UIStrokeTextBox.Transparency = 0.2
                UIStrokeTextBox.Thickness = 1
                UIStrokeTextBox.Parent = Textbox

                local function persist_text(text, invoke_callback)
                    TextboxManager._text = tostring(text or "")
                    Library._config._flags[settings.flag] = TextboxManager._text
                    Config.save(tostring(game.GameId), Library._config)

                    if invoke_callback then
                        local callback = settings and settings.callback
                        if type(callback) == "function" then
                            local ok, err = pcall(callback, TextboxManager._text)
                            if not ok then
                ...
                function TextboxManager:update_text(text, invoke_callback)
                    local next_text = tostring(text or "")
                    Textbox.Text = next_text
                    if Textbox:IsFocused() then
                        Textbox.PlaceholderText = ""
                    else
                        Textbox.PlaceholderText = (next_text == "" and placeholder_text or "")
                    end
                    persist_text(next_text, invoke_callback ~= false)
                end
                ...
                local function refresh_placeholder()
                    if Textbox:IsFocused() then
                        ...
                        Textbox.PlaceholderText = (Textbox.Text == "" and placeholder_text or "")
                    end
                end
                ...
                Textbox.FocusLost:Connect(function(enter_pressed)
                    persist_text(Textbox.Text, true)
                    UIStrokeTextBox.Transparency = 0.2
                    UIStrokeTextBox.Thickness = 1
                    refresh_placeholder()

                    if enter_pressed then
                        local enter_callback = settings and settings.enter_callback
                        if type(enter_callback) == "function" then
                            local ok, err = pcall(enter_callback, TextboxManager._text)
                            if not ok then
                                warn("textbox enter callback error:", err)
                            end
                        end
                    end
                end)
                ...
                persist_text(TextboxManager._text, false)
                refresh_placeholder()
            ...
            function ModuleManager:create_checkbox(settings)
                ...
                local function emit_checkbox_callback()
                    local callback = settings and settings.callback
                    if type(callback) ~= "function" then
                        return
                    end
                    local ok, err = pcall(callback, ModuleManager._state and CheckboxManager._state or false)
                    if not ok then
                        warn("checkbox callback error:", err)
                    end
                end
                ...
                ModuleManager:register_control_sync(emit_checkbox_callback)
            ...
            function ModuleManager:create_slider(settings)
                ...
                function SliderManager:set_percentage(percentage)
                    ...
                    do
                        local callback = settings and settings.callback
                        if type(callback) == "function" then
                            local ok, err = pcall(callback, number_threshold)
                            if not ok then
                ...
                Value.FocusLost:Connect(function()
                    local parsed_number = tonumber(Value.Text)
                    if not parsed_number then
                        parsed_number = Library._config._flags[settings.flag]
                        if parsed_number == nil then
                            parsed_number = settings.value or settings.minimum_value or 0
                        end
                    end

                    SliderManager:set_percentage(parsed_number)
            ...
            function ModuleManager:create_feature(settings)
                ...
                FeatureStroke.Color = ThemeColors.Primary
                ...
                local function emit_feature_callback()
                    local callback = settings and settings.callback
                    if type(callback) ~= "function" then
                        return
                    end
                    local ok, err = pcall(callback, ModuleManager._state and checked or false)
                    if not ok then
                        warn("feature callback error:", err)
                    end
                end
                ...
                if not settings.disablecheck then
                    ...
                    UseF_Var = function()
                        ...
                        local button_callback = settings and settings.button_callback
                        if type(button_callback) == "function" then
                            local ok, err = pcall(button_callback)
                            if not ok then
                                warn("feature button callback error:", err)
                            end
                        end
                    end
                end
                ...
                if not settings.disablecheck then
                    emit_feature_callback()
                    ModuleManager:register_control_sync(emit_feature_callback)
                end
    ...
    do
        local PROFILE_NAME_FLAG = "__river_cfg_profile_name"
        local PROFILE_SELECTED_FLAG = "__river_cfg_selected"
        local AUTOLOAD_FLAG = "__river_cfg_autoload"
        local THEME_FLAG = "__river_cfg_theme"

        if typeof(Library._config._flags[PROFILE_NAME_FLAG]) ~= "string" then
            Library._config._flags[PROFILE_NAME_FLAG] = sanitize_profile_name(
                Library._config._library.selected_profile or "default"
            )
        end

        if typeof(Library._config._flags[AUTOLOAD_FLAG]) ~= "boolean" then
            Library._config._flags[AUTOLOAD_FLAG] = Library._config._library.autoload == true
        end

        if typeof(Library._config._flags[THEME_FLAG]) ~= "string" then
            local stored_theme = Library._config._library.theme
            if typeof(stored_theme) ~= "string" or not ThemePresets[stored_theme] then
                stored_theme = "Crimson"
            end
            Library._config._flags[THEME_FLAG] = stored_theme
            Library._config._library.theme = stored_theme
            Config.save(tostring(game.GameId), Library._config)
        end

        local function notify_config(text)
        ...
        local function get_profile_options()
            local profiles = self:get_profile_list()
            if #profiles == 0 then
                profiles = { "default" }
            end
            return profiles
        ...
        local function get_selected_profile()
            local selected = Library._config._flags[PROFILE_SELECTED_FLAG]
            if typeof(selected) == "string" and selected ~= "" then
                return sanitize_profile_name(selected)
            end

            local typed = Library._config._flags[PROFILE_NAME_FLAG]
            if typeof(typed) == "string" and typed ~= "" then
                return sanitize_profile_name(typed)
            end

            local stored = Library._config._library.selected_profile
            if typeof(stored) == "string" and stored ~= "" then
                return sanitize_profile_name(stored)
            end

            return "default"
        end

        local config_tab = self:create_tab("Configs", "settings-2", {
            bottom = true,
            no_auto_select = true
        })
        local config_module = config_tab:create_module({
            title = "Profile Manager",
            description = "Save / load / autoload named configs",
            flag = "__river_config_module",
            section = "left"
        })

        local info_module = config_tab:create_module({
            title = "Script Info",
            description = "Status, credits and community",
            flag = "__river_info_module",
            section = "right"
        })

        info_module:create_divider({
            showtopic = true,
            title = "Status"
        })

        info_module:create_text({
            text = "Current Status: Undetected",
            customScale = 45
        })
        ...
        info_module:create_text({
            text = "discord.gg/stealthhub",
            customScale = 45
        })

        config_module:create_textbox({
            title = "Profile Name",
            flag = PROFILE_NAME_FLAG,
            placeholder = "default",
            callback = function(text)
                local normalized = sanitize_profile_name(text)
                Library._config._flags[PROFILE_NAME_FLAG] = normalized
                Config.save(tostring(game.GameId), Library._config)
            end
        })
        ...
        local function on_profile_selected(option)
            local profile_name = sanitize_profile_name((typeof(option) == "string" and option) or tostring(option))

            Library._config._flags[PROFILE_SELECTED_FLAG] = profile_name
            Library._config._flags[PROFILE_NAME_FLAG] = profile_name
            Library._config._library.selected_profile = profile_name

            if Library._config._library.autoload then
                Library._config._library.autoload_profile = profile_name
            end

            Config.save(tostring(game.GameId), Library._config)
        end

        local function refresh_profiles_dropdown(preferred_profile)
            if not dropdown_manager then
                return
            end

            dropdown_manager = dropdown_manager:New({
                title = "Saved Profiles",
                flag = PROFILE_SELECTED_FLAG,
            ...
            if preferred_profile then
                pcall(function()
                    dropdown_manager:update(sanitize_profile_name(preferred_profile))
                end)
            end
        end

        dropdown_manager = config_module:create_dropdown({
            title = "Saved Profiles",
            flag = PROFILE_SELECTED_FLAG,
            options = get_profile_options(),
            callback = on_profile_selected
        ...
        pcall(function()
            dropdown_manager:update(get_selected_profile())
        end)

        local theme_initialized = false
        local theme_dropdown = config_module:create_dropdown({
            title = "Theme",
            flag = THEME_FLAG,
            options = self:get_theme_names(),
            callback = function(option)
                local desired_theme = (typeof(option) == "string" and option) or tostring(option)
                local applied_theme = self:apply_theme(desired_theme, true, true)
                Library._config._flags[THEME_FLAG] = applied_theme

                if theme_initialized then
                    notify_config("Theme applied: " .. applied_theme)
                end
        ...
        pcall(function()
            theme_dropdown:update(Library._config._flags[THEME_FLAG])
        end)

        config_module:create_checkbox({
            title = "Autoload Selected",
            flag = AUTOLOAD_FLAG,
            callback = function(state)
                local selected_profile = get_selected_profile()

                Library._config._library.autoload = state
                if state then
                    Library._config._library.autoload_profile = selected_profile
                else
                    Library._config._library.autoload_profile = nil
                end
        ...
        config_module:create_feature({
            title = "Save / Create",
            flag = "__river_cfg_feature_save",
            disablecheck = true,
            button_callback = function()
                local profile_name = sanitize_profile_name(Library._config._flags[PROFILE_NAME_FLAG] or "default")
                Library._config._flags[PROFILE_NAME_FLAG] = profile_name

                local ok, saved_name, err = self:save_profile(profile_name)
                if ok then
                    Library._config._flags[PROFILE_SELECTED_FLAG] = saved_name
                    refresh_profiles_dropdown(saved_name)
                    notify_config("Saved profile: " .. saved_name)
                else
                    notify_config("Failed to save profile: " .. tostring(err))
                end
            end
        })
        ...
        config_module:create_feature({
            title = "Load Selected",
            flag = "__river_cfg_feature_load",
            disablecheck = true,
            button_callback = function()
                local profile_name = get_selected_profile()
                local ok, loaded_name, err = self:load_profile(profile_name)

                if ok then
                    Library._config._flags[PROFILE_NAME_FLAG] = loaded_name
                    Library._config._flags[PROFILE_SELECTED_FLAG] = loaded_name
                    refresh_profiles_dropdown(loaded_name)
                    notify_config("Loaded profile: " .. loaded_name .. " (re-execute to refresh UI state)")
                else
                    notify_config("Failed to load profile: " .. tostring(err))
                end
            end
        })
        ...
        config_module:create_feature({
            title = "Refresh List",
            flag = "__river_cfg_feature_refresh",
            disablecheck = true,
            button_callback = function()
                refresh_profiles_dropdown(get_selected_profile())
                notify_config("Profile list refreshed")
            end
        })

        config_module:create_text({
            text = "Theme applies instantly. Profile load updates values; re-execute for full state sync.",
            customScale = 45
        })

        -- Force one closed/open cycle after all controls are added so contents are always visible.
        config_module:change_state(false)
        config_module:change_state(true)
        info_module:change_state(false)
        info_module:change_state(true)
    end
...
-- Adicionar funÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o SendNotification ÃƒÆ’Ã‚Â  Library
Library.SendNotification = Library.SendNotification

-- Adicionar funÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o create_refresh_dropdown ÃƒÆ’Ã‚Â  Library
